...
get_default_exploratory_csv_prefix() {
  ...
  if [[ "$scenario_users" == "1" && "$scenario_spawn_rate" == "1" && "$scenario_run_time" == "1m" ]]; then
    printf '%s/results/experimental-cycles/C0/benchmark/exploratory/E1_smoke_single_user/locust-smoke' "$repository_root"
    return
  fi

  if [[ "$scenario_users" == "2" && "$scenario_spawn_rate" == "1" && "$scenario_run_time" == "1m" ]]; then
    printf '%s/results/experimental-cycles/C0/benchmark/exploratory/E2_low_load_two_users/locust-low_load' "$repository_root"
    return
  fi

  if [[ "$scenario_users" == "4" && "$scenario_spawn_rate" == "2" && "$scenario_run_time" == "1m" ]]; then
    printf '%s/results/experimental-cycles/C0/benchmark/exploratory/E3_small_concurrency_four_users/locust-small_concurrency' "$repository_root"
    return
  fi

  printf '%s/results/experimental-cycles/C0/benchmark/exploratory/manual_run/locust-exploratory' "$repository_root"
}
