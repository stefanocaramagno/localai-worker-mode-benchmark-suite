...
function Get-DefaultExploratoryCsvPrefix {
    ...
    if ($ScenarioUsers -eq 1 -and $ScenarioSpawnRate -eq 1 -and $ScenarioRunTime -eq "1m") {
        return Join-Path $RepositoryRoot "results\experimental-cycles\C0\benchmark\exploratory\E1_smoke_single_user\locust-smoke"
    }

    if ($ScenarioUsers -eq 2 -and $ScenarioSpawnRate -eq 1 -and $ScenarioRunTime -eq "1m") {
        return Join-Path $RepositoryRoot "results\experimental-cycles\C0\benchmark\exploratory\E2_low_load_two_users\locust-low_load"
    }

    if ($ScenarioUsers -eq 4 -and $ScenarioSpawnRate -eq 2 -and $ScenarioRunTime -eq "1m") {
        return Join-Path $RepositoryRoot "results\experimental-cycles\C0\benchmark\exploratory\E3_small_concurrency_four_users\locust-small_concurrency"
    }

    return Join-Path $RepositoryRoot "results\experimental-cycles\C0\benchmark\exploratory\manual_run\locust-exploratory"
}
