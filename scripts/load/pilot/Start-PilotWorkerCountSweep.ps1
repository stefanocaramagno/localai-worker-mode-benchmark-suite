...
function Wait-ForWorkerPodsScheduled {
    ...
    throw "Timeout while waiting for RPC worker pod scheduling in namespace '$Namespace'. Expected: $ExpectedWorkerCount, observed: $observed, with populated nodeName: $scheduled."
}
...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) { $OutputRoot = Join-Path $repoRoot "results\experimental-cycles\C0\benchmark\pilot\worker-count" }
...
if ($referenceBaselineId -ne [string]$baselineConfigData.baselineId) {
    throw "The worker-count scenario $Scenario requires baseline $referenceBaselineId but the provided file exposes $($baselineConfigData.baselineId)."
}
...
if ([string]$modelScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.modelScenario -or [string]$modelScenarioData.RawConfig.modelName -ne [string]$baselineConfigData.resolvedModelName -or [string]$modelScenarioData.RawConfig.serverManifest -ne [string]$baselineConfigData.serverManifest) {
    throw "The worker baseline is not consistent with the reference model scenario."
}
...
if ([string]$placementScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.placementScenario -or [string]$placementScenarioData.RawConfig.placementType -ne [string]$baselineConfigData.resolvedPlacementType -or [string]$placementScenarioData.RawConfig.topologyDir -ne [string]$baselineConfigData.topologyDir) {
    throw "The worker baseline is not consistent with the reference placement scenario."
}

if ([string]$workloadScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.workloadScenario) {
    throw "The worker baseline is not consistent with the reference workload scenario."
}

if (-not [string]::IsNullOrWhiteSpace($Model) -and $Model -ne [string]$baselineConfigData.resolvedModelName) {
    throw "The worker-count sweep is anchored to baseline $($baselineConfigData.baselineId). The requested model ($Model) does not match the fixed baseline model ($($baselineConfigData.resolvedModelName))."
}
...
if (-not [string]::IsNullOrWhiteSpace($Prompt) -and $Prompt -ne [string]$baselineConfigData.prompt) {
    throw "The worker-count sweep is anchored to baseline $($baselineConfigData.baselineId). The requested prompt does not match the fixed baseline prompt."
}
...
if ($AutoApplyK8s) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($Namespace)) {
        Write-Host "Waiting for RPC worker pod scheduling after auto-apply..." -ForegroundColor Yellow
        try {
            ...
            foreach ($pod in $scheduledWorkerPods) {
                ...
                $nodeName = if ($null -ne $nodeNameProperty) { [string]$nodeNameProperty.Value } else { '<unassigned>' }
                Write-Host " - $podName scheduled on $nodeName" -ForegroundColor DarkYellow
            }
        }
        catch {
            ...
            if ($isUnsupported) {
                $unsupportedReason = "Worker-count scenario unsupported under the current constraints of the fixed baseline: placement '$($baselineConfigData.resolvedPlacementType)' must remain unchanged and, after auto-apply and the configured stabilization window, at least one RPC pod required by the scenario remains unschedulable in the current cluster due to resource and/or affinity constraints."
                ...
                Write-Host "" 
                Write-Host "Scenario unsupported under the current constraints." -ForegroundColor Yellow
