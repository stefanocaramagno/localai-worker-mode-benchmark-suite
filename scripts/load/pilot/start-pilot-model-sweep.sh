...
validate_model_scenario() {
  case "$1" in
    M1|M2|M3|M4) ;;
    *)
      echo "Unsupported model scenario: $1" >&2
      exit 1
      ;;
  esac
}
...
PY
  normalize_artifact_file "$json_path"
...
[[ -n "$OUTPUT_ROOT" ]] || OUTPUT_ROOT="$REPO_ROOT/results/experimental-cycles/C0/benchmark/pilot/models"
...
[[ -f "$MODEL_SCENARIO_FILE" ]] || { echo "The scenario configuration file does not exist: $MODEL_SCENARIO_FILE" >&2; exit 1; }
...
if [[ "$REFERENCE_BASELINE_ID" != "$BASELINE_ID" ]]; then
  echo "The model scenario $SCENARIO requires baseline $REFERENCE_BASELINE_ID but the provided file exposes $BASELINE_ID." >&2
  exit 1
fi
...
if [[ "${JSON_VALUES[1]}" != "$BASELINE_WORKER_SCENARIO" || "${JSON_VALUES[3]}" != "$BASELINE_RESOLVED_WORKER_COUNT" ]]; then
  echo "The model baseline is not consistent with the reference worker scenario." >&2
  exit 1
fi
...
if [[ "${JSON_VALUES[1]}" != "$BASELINE_PLACEMENT_SCENARIO" || "${JSON_VALUES[2]}" != "$BASELINE_PLACEMENT_TYPE" || "${JSON_VALUES[3]}" != "$BASELINE_TOPOLOGY_DIR_REL" ]]; then
  echo "The model baseline is not consistent with the reference placement scenario." >&2
  exit 1
fi
...
if [[ "${JSON_VALUES[1]}" != "$BASELINE_WORKLOAD_SCENARIO" ]]; then
  echo "The model baseline is not consistent with the reference workload scenario." >&2
  exit 1
fi

if [[ -n "$MODEL" && "$MODEL" != "$RESOLVED_MODEL_FROM_FILE" ]]; then
  echo "The model sweep is anchored to scenario $SCENARIO. The requested model ($MODEL) does not match the resolved scenario model ($RESOLVED_MODEL_FROM_FILE)." >&2
  exit 1
fi
MODEL="$RESOLVED_MODEL_FROM_FILE"

if [[ -n "$PROMPT" && "$PROMPT" != "$BASELINE_PROMPT" ]]; then
  echo "The model sweep is anchored to baseline $BASELINE_ID. The requested prompt does not match the fixed baseline prompt." >&2
  exit 1
fi
...
if [[ "$API_SMOKE_ENABLED" == "true" ]]; then
  ...
  if [[ $API_SMOKE_EXIT_CODE -eq 42 ]]; then
    unsupported_reason="Model scenario operationally unsupported under the current constraints of the fixed baseline: model '$MODEL' did not return a smoke-test response within ${BASELINE_REQUEST_TIMEOUT_SECONDS} seconds in the current LocalAI worker-mode configuration."
    ...
    echo
    echo "Scenario operationally unsupported."
