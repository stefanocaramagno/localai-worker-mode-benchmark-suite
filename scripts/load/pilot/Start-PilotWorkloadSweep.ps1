...
function Get-ScenarioConfig {
    ...
    $scenarioFile = Join-Path $ConfigRoot ("{0}.json" -f $ScenarioName)
    if (-not (Test-Path $scenarioFile)) {
        throw "The scenario configuration file does not exist: $scenarioFile"
    }

    $rawConfig = Get-Content -Path $scenarioFile -Raw -Encoding UTF8 | ConvertFrom-Json
    $requiredProperties = @("scenarioId", "purpose", "users", "spawnRate", "runTime", "outputSubdir", "referenceBaselineId")

    foreach ($propertyName in $requiredProperties) {
        if (-not ($rawConfig.PSObject.Properties.Name -contains $propertyName)) {
            throw "The scenario configuration file '$scenarioFile' does not contain the required property '$propertyName'."
        }
    }

    return @{
        ScenarioFile = $scenarioFile
        ScenarioId = [string]$rawConfig.scenarioId
        Purpose = [string]$rawConfig.purpose
        Users = [int]$rawConfig.users
        SpawnRate = [int]$rawConfig.spawnRate
        RunTime = [string]$rawConfig.runTime
        OutputSubdir = [string]$rawConfig.outputSubdir
        ReferenceBaselineId = [string]$rawConfig.referenceBaselineId
    }
}
...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $repoRoot "results\experimental-cycles\C0\benchmark\pilot\workload"
}

if ([string]::IsNullOrWhiteSpace($ScenarioConfigRoot)) {
    $ScenarioConfigRoot = Join-Path $repoRoot "config\scenarios\pilot\workload"
}
...
if (-not (Test-Path $BaselineConfig)) {
    throw "The baseline file does not exist: $BaselineConfig"
}
...
$scenarioConfig = Get-ScenarioConfig -ScenarioName $Scenario -ConfigRoot $ScenarioConfigRoot
$users = $scenarioConfig.Users
$spawnRate = $scenarioConfig.SpawnRate
$runTime = $scenarioConfig.RunTime
$purpose = $scenarioConfig.Purpose
$outputSubdir = $scenarioConfig.OutputSubdir
$referenceBaselineId = $scenarioConfig.ReferenceBaselineId
...
if ($referenceBaselineId -ne [string]$baselineConfigData.baselineId) {
    throw "The workload scenario $Scenario requires baseline $referenceBaselineId but the provided file exposes $($baselineConfigData.baselineId)."
}

$modelScenarioFile = Join-Path $repoRoot ("config\scenarios\pilot\models\{0}.json" -f $baselineConfigData.modelScenario)
$workerScenarioFile = Join-Path $repoRoot ("config\scenarios\pilot\worker-count\{0}.json" -f $baselineConfigData.workerScenario)
...
if ([string]$modelScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.modelScenario -or [string]$modelScenarioData.RawConfig.modelName -ne [string]$baselineConfigData.resolvedModelName -or [string]$modelScenarioData.RawConfig.serverManifest -ne [string]$baselineConfigData.serverManifest) {
    throw "The workload baseline is not consistent with the reference model scenario."
}
...
if ([string]$workerScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.workerScenario -or [int]$workerScenarioData.RawConfig.workerCount -ne [int]$baselineConfigData.resolvedWorkerCount) {
    throw "The workload baseline is not consistent with the reference worker scenario."
}
...
if ([string]$placementScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.placementScenario -or [string]$placementScenarioData.RawConfig.placementType -ne [string]$baselineConfigData.resolvedPlacementType -or [string]$placementScenarioData.RawConfig.topologyDir -ne [string]$baselineConfigData.topologyDir) {
    throw "The workload baseline is not consistent with the reference placement scenario."
}

if (-not [string]::IsNullOrWhiteSpace($Model) -and $Model -ne [string]$baselineConfigData.resolvedModelName) {
    throw "The workload sweep is anchored to baseline $($baselineConfigData.baselineId). The requested model ($Model) does not match the fixed baseline model ($($baselineConfigData.resolvedModelName))."
}
...
if (-not [string]::IsNullOrWhiteSpace($Prompt) -and $Prompt -ne [string]$baselineConfigData.prompt) {
    throw "The workload sweep is anchored to baseline $($baselineConfigData.baselineId). The requested prompt does not match the fixed baseline prompt."
}
...
$outputDir = Join-Path $OutputRoot $outputSubdir
...
$warmUpLocustArgs = @(
    ...
    "--run-time", $phasePlan.WarmUpDuration,
...
$measurementLocustArgs = @(
    ...
    "--run-time", $phasePlan.MeasurementDuration,
...
Write-MetricSetFiles -MetricSetProfile $metricSetProfile -ManifestPath $metricSetPaths.ManifestPath -TextPath $metricSetPaths.TextPath -LauncherName "Start-PilotWorkloadSweep" -RunId $runId -MeasurementCsvPrefix $phasePlan.MeasurementCsvPrefix -ClientSourceArtifacts $metricSetClientArtifacts -ClusterSourceArtifacts $metricSetClusterArtifacts
...
Write-Host "Scenario cfg                 : $($scenarioConfig.ScenarioFile)"
Write-Host "Scenario users               : $users"
Write-Host "Scenario spawn rate          : $spawnRate"
