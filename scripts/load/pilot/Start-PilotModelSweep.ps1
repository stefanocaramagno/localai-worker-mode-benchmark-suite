...
function Get-JsonScenarioConfig {
    ...
    if (-not (Test-Path $resolvedConfigFile)) {
        throw "The JSON file does not exist: $resolvedConfigFile"
    }
    ...
    foreach ($propertyName in $RequiredProperties) {
        if (-not ($rawConfig.PSObject.Properties.Name -contains $propertyName)) {
            throw "The JSON file '$resolvedConfigFile' does not contain the required property '$propertyName'."
        }
    }
...
function Invoke-K8sApplyTarget {
    ...
    if (-not (Test-CommandAvailable -CommandName "kubectl")) {
        throw "kubectl is not available in PATH. Unable to apply Kubernetes targets automatically."
    }
    ...
    Write-Host ("Automatic Kubernetes target apply: kubectl " + ($kubectlArgs -join " ")) -ForegroundColor Yellow
    & kubectl @kubectlArgs

    if ($LASTEXITCODE -ne 0) {
        throw "Kubernetes apply failed for target: $Path (exit code: $LASTEXITCODE)"
    }
}

function Write-UnsupportedModelScenarioArtifacts {
    param(
    ...
    Write-PortableArtifactJson -Path $jsonPath -Payload $payload -Depth 8

    $lines = New-Object System.Collections.Generic.List[string]
    $lines.Add('=============================================') | Out-Null
    $lines.Add(' Unsupported Scenario Report') | Out-Null
    $lines.Add('=============================================') | Out-Null
    $lines.Add('Family                : model') | Out-Null
    ...
    $lines.Add(("Model                 : {0}" -f $ModelName)) | Out-Null
    $lines.Add(("Timeout (s)           : {0}" -f $TimeoutSeconds)) | Out-Null
...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) { $OutputRoot = Join-Path $repoRoot "results\experimental-cycles\C0\benchmark\pilot\models" }
...
if ([string]::IsNullOrWhiteSpace($PrecheckConfig)) { $PrecheckConfig = Join-Path $repoRoot "config\precheck\profiles\TC_C0_HISTORICAL_FIXED_CLUSTER.json" }
if ([string]::IsNullOrWhiteSpace($PhaseConfig)) { $PhaseConfig = Join-Path $repoRoot "config\phases\profiles\WM_STANDARD_WARMUP_MEASUREMENT.json" }
if ([string]::IsNullOrWhiteSpace($ProtocolConfig)) { $ProtocolConfig = Join-Path $repoRoot "config\protocol\profiles\EP_STANDARD_BENCHMARK_PROTOCOL.json" }
if ([string]::IsNullOrWhiteSpace($ClusterCaptureConfig)) { $ClusterCaptureConfig = Join-Path $repoRoot "config\cluster-capture\profiles\CS_STANDARD_CLUSTER_CAPTURE.json" }
if ([string]::IsNullOrWhiteSpace($MetricSetConfig)) { $MetricSetConfig = Join-Path $repoRoot "config\metric-set\profiles\MS_STANDARD_BENCHMARK_METRICS.json" }
...
if (-not (Test-Path $LocustFile)) { throw "The specified Locust file does not exist: $LocustFile" }
if (-not (Test-Path $BaselineConfig)) { throw "The baseline file does not exist: $BaselineConfig" }
if (-not $SkipPrecheck -and -not (Test-Path $precheckScript)) { throw "The pre-check script does not exist: $precheckScript" }
if (-not (Test-Path $PhaseConfig)) { throw "The warm-up/measurement profile file does not exist: $PhaseConfig" }
if (-not (Test-Path $ProtocolConfig)) { throw "The protocol file does not exist: $ProtocolConfig" }
if (-not (Test-Path $ClusterCaptureConfig)) { throw "The cluster-side collection file does not exist: $ClusterCaptureConfig" }
if (-not (Test-Path $MetricSetConfig)) { throw "The metric-set file does not exist: $MetricSetConfig" }
if (-not (Test-Path $clusterCaptureScript)) { throw "The cluster-side collection script does not exist: $clusterCaptureScript" }
if (-not $DryRun -and -not (Test-CommandAvailable -CommandName "locust")) { throw "Locust is not available in PATH. Install Locust or verify the Python environment." }
...
if ($apiSmokeEnabled -and -not (Test-Path $apiSmokeScript)) { throw "The API smoke script does not exist: $apiSmokeScript" }

$modelScenarioData = Get-JsonScenarioConfig `
    -ScenarioName $Scenario `
    -ConfigRoot $ModelScenarioConfigRoot `
    -RequiredProperties @("scenarioId", "purpose", "modelName", "outputSubdir", "referenceBaselineId", "serverManifest")
$modelConfig = $modelScenarioData.RawConfig
$referenceBaselineId = [string]$modelConfig.referenceBaselineId
...
$resolvedBaseUrl = if ([string]::IsNullOrWhiteSpace($BaseUrl)) {
    [string]$baselineConfigData.baseUrl
}
else {
    [string]$BaseUrl
}

$requestTimeoutSeconds = [int]$baselineConfigData.requestTimeoutSeconds

if ($referenceBaselineId -ne [string]$baselineConfigData.baselineId) {
    throw "The model scenario $Scenario requires baseline $referenceBaselineId but the provided file exposes $($baselineConfigData.baselineId)."
}

$workerScenarioFile = Join-Path $WorkerCountScenarioConfigRoot ("{0}.json" -f $baselineConfigData.workerScenario)
$placementScenarioFile = Join-Path $PlacementScenarioConfigRoot ("{0}.json" -f $baselineConfigData.placementScenario)
...
if ([string]$workerScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.workerScenario -or [int]$workerScenarioData.RawConfig.workerCount -ne [int]$baselineConfigData.resolvedWorkerCount) {
    throw "The model baseline is not consistent with the reference worker scenario."
}
...
if ([string]$placementScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.placementScenario -or [string]$placementScenarioData.RawConfig.placementType -ne [string]$baselineConfigData.resolvedPlacementType -or [string]$placementScenarioData.RawConfig.topologyDir -ne [string]$baselineConfigData.topologyDir) {
    throw "The model baseline is not consistent with the reference placement scenario."
}

if ([string]$workloadScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.workloadScenario) {
    throw "The model baseline is not consistent with the reference workload scenario."
}

if (-not [string]::IsNullOrWhiteSpace($Model) -and $Model -ne [string]$modelConfig.modelName) {
    throw "The model sweep is anchored to scenario $Scenario. The requested model ($Model) does not match the resolved scenario model ($($modelConfig.modelName))."
}
$Model = [string]$modelConfig.modelName
$resolvedModelName = $Model

if (-not [string]::IsNullOrWhiteSpace($Prompt) -and $Prompt -ne [string]$baselineConfigData.prompt) {
    throw "The model sweep is anchored to baseline $($baselineConfigData.baselineId). The requested prompt does not match the fixed baseline prompt."
}
$Prompt = [string]$baselineConfigData.prompt

$topologyRoot = Join-Path $repoRoot ([string]$baselineConfigData.topologyDir -replace '/', '\')
...
foreach ($targetPath in $k8sApplyTargets) {
    if (-not (Test-K8sApplyTarget -Path $targetPath)) { throw "Kubernetes target not found or invalid: $targetPath" }
}

$outputDir = Join-Path $OutputRoot ([string]$modelConfig.outputSubdir)
...
$apiSmokeArgs = @($apiSmokeScript, "-BaseUrl", $resolvedBaseUrl, "-Model", $resolvedModelName, "-RequestTimeoutSeconds", ([string]$requestTimeoutSeconds), "-ExitUnsupportedOnTimeout", "-UnsupportedExitCode", "42")
...
$warmUpLocustArgs = @(
    "-f", $LocustFile, "-H", $resolvedBaseUrl, "--headless",
...
Write-MetricSetFiles -MetricSetProfile $metricSetProfile -ManifestPath $metricSetPaths.ManifestPath -TextPath $metricSetPaths.TextPath -LauncherName "Start-PilotModelSweep" -RunId $runId -MeasurementCsvPrefix $phasePlan.MeasurementCsvPrefix -ClientSourceArtifacts $metricSetClientArtifacts -ClusterSourceArtifacts $metricSetClusterArtifacts
...
Write-Host "Model                        : $resolvedModelName"
...
Write-Host ""
if (-not $SkipPrecheck) {
    Write-Host "Pre-check command (con output prefix):" -ForegroundColor Yellow
    Write-Host ($precheckScript + " " + ($precheckArgs -join " "))
    Write-Host "Pre-check output prefix: $($precheckArgs[3])"
    Write-Host "Pre-check JSON atteso  : $precheckJsonPath"
    Write-Host "Pre-check text atteso  : $precheckTextPath"
    Write-Host ""
}
...
Write-Host ""
if ($AutoApplyK8s) {
    Write-Host "Automatically applying the recommended Kubernetes targets before the run." -ForegroundColor Yellow
    foreach ($targetPath in $k8sApplyTargets) {
...
if ($apiSmokeEnabled) {
    & $apiSmokeScript -BaseUrl $resolvedBaseUrl -Model $resolvedModelName -RequestTimeoutSeconds ([int]$requestTimeoutSeconds) -ExitUnsupportedOnTimeout -UnsupportedExitCode 42
    $apiSmokeExitCode = $LASTEXITCODE
    if ($apiSmokeExitCode -eq 42) {
        $unsupportedReason = "Model scenario operationally unsupported under the current constraints of the fixed baseline: model '$resolvedModelName' did not return a smoke-test response within $([int]$requestTimeoutSeconds) seconds in the current LocalAI worker-mode configuration."
        $unsupportedArtifacts = Write-UnsupportedModelScenarioArtifacts -OutputPrefix $csvPrefix -Scenario $Scenario -Replica $Replica -ModelName $resolvedModelName -TimeoutSeconds ([int]$requestTimeoutSeconds) -Reason $unsupportedReason -Namespace $Namespace
        Write-Host ""
        Write-Host "Scenario operationally unsupported." -ForegroundColor Yellow
