...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) { $OutputRoot = Join-Path $repoRoot "results\pilot\models" }
...
if ($referenceBaselineId -ne [string]$baselineConfigData.baselineId) {
    throw "Lo scenario model $Scenario richiede la baseline $referenceBaselineId ma il file fornito espone $($baselineConfigData.baselineId)."
}
...
if ([string]$workerScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.workerScenario -or [int]$workerScenarioData.RawConfig.workerCount -ne [int]$baselineConfigData.resolvedWorkerCount) {
    throw "La baseline model non è coerente con lo scenario worker di riferimento."
}
...
if ([string]$placementScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.placementScenario -or [string]$placementScenarioData.RawConfig.placementType -ne [string]$baselineConfigData.resolvedPlacementType -or [string]$placementScenarioData.RawConfig.topologyDir -ne [string]$baselineConfigData.topologyDir) {
    throw "La baseline model non è coerente con lo scenario placement di riferimento."
}

if ([string]$workloadScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.workloadScenario) {
    throw "La baseline model non è coerente con lo scenario workload di riferimento."
}

if (-not [string]::IsNullOrWhiteSpace($Model) -and $Model -ne [string]$modelConfig.modelName) {
    throw "Il model sweep è ancorato allo scenario $Scenario. Il modello richiesto ($Model) non coincide con il modello risolto dello scenario ($($modelConfig.modelName))."
}
...
if (-not [string]::IsNullOrWhiteSpace($Prompt) -and $Prompt -ne [string]$baselineConfigData.prompt) {
    throw "Il model sweep è ancorato alla baseline $($baselineConfigData.baselineId). Il prompt richiesto non coincide con il prompt fisso di baseline."
}
...
Write-Host ""
if (-not $SkipPrecheck) {
    Write-Host "Comando pre-check (con output prefix):" -ForegroundColor Yellow
...
if ($apiSmokeEnabled) {
    ...
    if ($apiSmokeExitCode -eq 42) {
        $unsupportedReason = "Scenario model non supportato operativamente sotto i vincoli correnti della baseline fissata: il modello '$resolvedModelName' non ha restituito una risposta di smoke test entro $([int]$requestTimeoutSeconds) secondi nella configurazione corrente di LocalAI worker mode."
        ...
        Write-Host ""
        Write-Host "Scenario non supportato operativamente." -ForegroundColor Yellow
