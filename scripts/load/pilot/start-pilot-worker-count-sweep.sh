...
validate_worker_scenario() {
  case "$1" in
    W1|W2|W3|W4) ;;
    *)
      echo "Scenario worker non supportato: $1" >&2
      exit 1
      ;;
  esac
}
...
if [[ -z "$OUTPUT_ROOT" ]]; then
  OUTPUT_ROOT="$REPO_ROOT/results/pilot/worker-count"
fi
...
if [[ ! -f "$WORKER_SCENARIO_FILE" ]]; then
  echo "Il file di configurazione dello scenario non esiste: $WORKER_SCENARIO_FILE" >&2
  exit 1
fi
...
if [[ "$REFERENCE_BASELINE_ID" != "$BASELINE_ID" ]]; then
  echo "Lo scenario worker $SCENARIO richiede la baseline $REFERENCE_BASELINE_ID ma il file fornito espone $BASELINE_ID." >&2
  exit 1
fi
...
if [[ "${JSON_VALUES[1]}" != "$BASELINE_MODEL_SCENARIO" || "${JSON_VALUES[2]}" != "$BASELINE_MODEL_NAME" || "${JSON_VALUES[3]}" != "$BASELINE_SERVER_MANIFEST_REL" ]]; then
  echo "La baseline worker non è coerente con lo scenario model di riferimento." >&2
  exit 1
fi
...
if [[ "${JSON_VALUES[1]}" != "$BASELINE_PLACEMENT_SCENARIO" || "${JSON_VALUES[2]}" != "$BASELINE_PLACEMENT_TYPE" || "${JSON_VALUES[3]}" != "$BASELINE_TOPOLOGY_DIR_REL" ]]; then
  echo "La baseline worker non è coerente con lo scenario placement di riferimento." >&2
  exit 1
fi
...
if [[ "${JSON_VALUES[1]}" != "$BASELINE_WORKLOAD_SCENARIO" ]]; then
  echo "La baseline worker non è coerente con lo scenario workload di riferimento." >&2
  exit 1
fi

if [[ -n "$MODEL" && "$MODEL" != "$BASELINE_MODEL_NAME" ]]; then
  echo "Il worker count sweep è ancorato alla baseline $BASELINE_ID. Il modello richiesto ($MODEL) non coincide con il modello fisso di baseline ($BASELINE_MODEL_NAME)." >&2
  exit 1
fi
MODEL="$BASELINE_MODEL_NAME"

if [[ -n "$PROMPT" && "$PROMPT" != "$BASELINE_PROMPT" ]]; then
  echo "Il worker count sweep è ancorato alla baseline $BASELINE_ID. Il prompt richiesto non coincide con il prompt fisso di baseline." >&2
  exit 1
fi
