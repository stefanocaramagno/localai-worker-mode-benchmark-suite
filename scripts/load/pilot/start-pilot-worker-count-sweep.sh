...
print_usage() {
  cat <<'USAGE'
Usage:
  ./start-pilot-worker-count-sweep.sh --scenario W1|W2|W3|W4 --replica A|B|C [options]
...
validate_worker_scenario() {
  case "$1" in
    W1|W2|W3|W4) ;;
    *)
      echo "Unsupported worker scenario: $1" >&2
      exit 1
      ;;
  esac
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --output-root|-OutputRoot)
      ...
      shift 2
      ;;
    --worker-count-scenario-config-root)
      WORKER_COUNT_SCENARIO_CONFIG_ROOT="$2"
      shift 2
      ;;
    --workload-scenario-config-root|-WorkloadScenarioConfigRoot)
      WORKLOAD_SCENARIO_CONFIG_ROOT="$2"
      shift 2
      ;;
...
if [[ -z "$SCENARIO" ]]; then
  echo "The Scenario parameter is required." >&2
  print_usage >&2
  exit 1
fi
...
validate_worker_scenario "$SCENARIO"
...
if [[ -z "$OUTPUT_ROOT" ]]; then
  OUTPUT_ROOT="$REPO_ROOT/results/experimental-cycles/C0/benchmark/pilot/worker-count"
fi

if [[ -z "$WORKER_COUNT_SCENARIO_CONFIG_ROOT" ]]; then
  WORKER_COUNT_SCENARIO_CONFIG_ROOT="$REPO_ROOT/config/scenarios/pilot/worker-count"
fi

if [[ -z "$WORKLOAD_SCENARIO_CONFIG_ROOT" ]]; then
  WORKLOAD_SCENARIO_CONFIG_ROOT="$REPO_ROOT/config/scenarios/pilot/workload"
fi
...
if [[ -z "$PHASE_CONFIG" ]]; then
  PHASE_CONFIG="$REPO_ROOT/config/phases/profiles/WM_STANDARD_WARMUP_MEASUREMENT.json"
fi
...
if [[ ! -f "$LOCUST_FILE" ]]; then
  echo "The specified Locust file does not exist: $LOCUST_FILE" >&2
  exit 1
fi

if [[ ! -f "$BASELINE_CONFIG" ]]; then
  echo "The baseline file does not exist: $BASELINE_CONFIG" >&2
  exit 1
fi

if [[ "$SKIP_PRECHECK" != true && ! -f "$PRECHECK_SCRIPT" ]]; then
  echo "The pre-check script does not exist: $PRECHECK_SCRIPT" >&2
  exit 1
fi

if [[ ! -f "$PHASE_HELPER" ]]; then
  echo "The phase helper script does not exist: $PHASE_HELPER" >&2
  exit 1
fi

if [[ ! -f "$PHASE_CONFIG" ]]; then
  echo "The warm-up/measurement profile file does not exist: $PHASE_CONFIG" >&2
  exit 1
fi

if [[ ! -f "$PROTOCOL_HELPER" ]]; then
  echo "The protocol helper script does not exist: $PROTOCOL_HELPER" >&2
  exit 1
fi
if [[ ! -f "$CLUSTER_CAPTURE_HELPER" ]]; then
  echo "The cluster-side collection helper script does not exist: $CLUSTER_CAPTURE_HELPER" >&2
  exit 1
fi
if [[ ! -f "$METRIC_SET_HELPER" ]]; then
  echo "The metric-set helper script does not exist: $METRIC_SET_HELPER" >&2
  exit 1
fi
if [[ ! -f "$METRIC_SET_CONFIG" ]]; then
  echo "The metric-set file does not exist: $METRIC_SET_CONFIG" >&2
  exit 1
fi

if [[ ! -f "$PROTOCOL_CONFIG" ]]; then
  echo "The protocol file does not exist: $PROTOCOL_CONFIG" >&2
  exit 1
fi
...
if [[ ! -f "$CLUSTER_CAPTURE_CONFIG" ]]; then
  echo "The cluster-side collection file does not exist: $CLUSTER_CAPTURE_CONFIG" >&2
  exit 1
fi

if [[ ! -f "$CLUSTER_CAPTURE_SCRIPT" ]]; then
  echo "The cluster-side collection script does not exist: $CLUSTER_CAPTURE_SCRIPT" >&2
  exit 1
fi
...
WORKER_SCENARIO_FILE="$WORKER_COUNT_SCENARIO_CONFIG_ROOT/${SCENARIO}.json"
if [[ ! -f "$WORKER_SCENARIO_FILE" ]]; then
  echo "The scenario configuration file does not exist: $WORKER_SCENARIO_FILE" >&2
  exit 1
fi
...
WORKER_TOPOLOGY_DIR_REL="$REPO_ROOT/config/scenarios/pilot/worker-count/${SCENARIO}.json"
get_json_properties "$WORKER_SCENARIO_FILE" "topologyDir"
TOPOLOGY_DIR_REL="${JSON_VALUES[1]}"
if [[ -z "$TOPOLOGY_DIR_REL" ]]; then
  TOPOLOGY_DIR_REL="infra/k8s/compositions/topology/colocated-sc-app-02-${SCENARIO,,}"
fi

get_json_properties "$BASELINE_CONFIG" "baselineId,purpose,modelScenario,resolvedModelName,placementScenario,resolvedPlacementType,workloadScenario,resolvedWorkload,topologyDir,serverManifest,namespaceManifest,storageManifest,prompt,temperature,requestTimeoutSeconds"
...
BASELINE_PLACEMENT_SCENARIO="${JSON_VALUES[5]}"
BASELINE_PLACEMENT_TYPE="${JSON_VALUES[6]}"
BASELINE_WORKLOAD_SCENARIO="${JSON_VALUES[7]}"
BASELINE_RESOLVED_WORKLOAD_JSON="${JSON_VALUES[8]}"
...
if [[ "$REFERENCE_BASELINE_ID" != "$BASELINE_ID" ]]; then
  echo "The worker-count scenario $SCENARIO requires baseline $REFERENCE_BASELINE_ID but the provided file exposes $BASELINE_ID." >&2
  exit 1
fi
...
if [[ "${JSON_VALUES[1]}" != "$BASELINE_MODEL_SCENARIO" || "${JSON_VALUES[2]}" != "$BASELINE_MODEL_NAME" || "${JSON_VALUES[3]}" != "$BASELINE_SERVER_MANIFEST_REL" ]]; then
  echo "The worker baseline is not consistent with the reference model scenario." >&2
  exit 1
fi
...
if [[ "${JSON_VALUES[1]}" != "$BASELINE_PLACEMENT_SCENARIO" || "${JSON_VALUES[2]}" != "$BASELINE_PLACEMENT_TYPE" || "${JSON_VALUES[3]}" != "$BASELINE_TOPOLOGY_DIR_REL" ]]; then
  echo "The worker baseline is not consistent with the reference placement scenario." >&2
  exit 1
fi
...
BASELINE_WORKLOAD_PURPOSE="${JSON_VALUES[2]}"
USERS="${JSON_VALUES[3]}"
SPAWN_RATE="${JSON_VALUES[4]}"
RUN_TIME="${JSON_VALUES[5]}"
if [[ "${JSON_VALUES[1]}" != "$BASELINE_WORKLOAD_SCENARIO" ]]; then
  echo "The worker baseline is not consistent with the reference workload scenario." >&2
  exit 1
fi

if [[ -n "$MODEL" && "$MODEL" != "$BASELINE_MODEL_NAME" ]]; then
  echo "The worker-count sweep is anchored to baseline $BASELINE_ID. The requested model ($MODEL) does not match the fixed baseline model ($BASELINE_MODEL_NAME)." >&2
  exit 1
fi
MODEL="$BASELINE_MODEL_NAME"

if [[ -n "$PROMPT" && "$PROMPT" != "$BASELINE_PROMPT" ]]; then
  echo "The worker-count sweep is anchored to baseline $BASELINE_ID. The requested prompt does not match the fixed baseline prompt." >&2
  exit 1
fi
...
metric_set_write_files "$METRIC_SET_MANIFEST_PATH" "$METRIC_SET_TEXT_PATH" "Start-PilotWorkerCountSweep" "$RUN_ID" "$PHASE_MEASUREMENT_CSV_PREFIX" "$METRIC_SET_CLIENT_SOURCE_ARTIFACTS_JSON" "$METRIC_SET_CLUSTER_SOURCE_ARTIFACTS_JSON"
...
protocol_write_files "$PROTOCOL_MANIFEST_PATH" "$PROTOCOL_TEXT_PATH" "Start-PilotWorkerCountSweep" "$RUN_ID" \
...
echo "============================================="
echo " Official Pilot Worker Count Sweep Launcher"
echo "============================================="
echo "Repository               : $REPO_ROOT"
echo "Worker scenario          : $SCENARIO"
...
echo "Workload purpose         : $BASELINE_WORKLOAD_PURPOSE"
