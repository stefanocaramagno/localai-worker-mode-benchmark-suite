...
validate_placement_scenario() {
  case "$1" in
    PL1|PL2) ;;
    *)
      echo "Scenario placement non supportato: $1" >&2
      exit 1
      ;;
  esac
}
...
[[ -n "$OUTPUT_ROOT" ]] || OUTPUT_ROOT="$REPO_ROOT/results/pilot/placement"
...
[[ -f "$PLACEMENT_SCENARIO_FILE" ]] || { echo "Il file di configurazione dello scenario non esiste: $PLACEMENT_SCENARIO_FILE" >&2; exit 1; }
...
[[ "$REFERENCE_BASELINE_ID" == "$BASELINE_ID" ]] || { echo "Lo scenario placement $SCENARIO richiede la baseline $REFERENCE_BASELINE_ID ma il file fornito espone $BASELINE_ID." >&2; exit 1; }
...
[[ -f "$MODEL_SCENARIO_FILE" ]] || { echo "Il file di configurazione dello scenario modello non esiste: $MODEL_SCENARIO_FILE" >&2; exit 1; }
...
[[ -f "$WORKER_SCENARIO_FILE" ]] || { echo "Il file di configurazione dello scenario worker non esiste: $WORKER_SCENARIO_FILE" >&2; exit 1; }
...
[[ -f "$WORKLOAD_SCENARIO_FILE" ]] || { echo "Il file di configurazione dello scenario workload non esiste: $WORKLOAD_SCENARIO_FILE" >&2; exit 1; }
...
[[ "$MODEL_SCENARIO_ID" == "$BASELINE_MODEL_SCENARIO" && "$MODEL_NAME_FROM_FILE" == "$BASELINE_RESOLVED_MODEL_NAME" ]] || { echo "La baseline placement non è coerente con lo scenario model di riferimento." >&2; exit 1; }
[[ "$WORKER_SCENARIO_ID" == "$BASELINE_WORKER_SCENARIO" && "$WORKER_COUNT" == "$BASELINE_RESOLVED_WORKER_COUNT" ]] || { echo "La baseline placement non è coerente con lo scenario worker di riferimento." >&2; exit 1; }
[[ "$WORKLOAD_SCENARIO_ID" == "$BASELINE_WORKLOAD_SCENARIO" ]] || { echo "La baseline placement non è coerente con lo scenario workload di riferimento." >&2; exit 1; }

if [[ -n "$MODEL" && "$MODEL" != "$BASELINE_RESOLVED_MODEL_NAME" ]]; then
  echo "Il placement sweep è ancorato alla baseline $BASELINE_ID. Il modello richiesto ($MODEL) non coincide con il modello fisso di baseline ($BASELINE_RESOLVED_MODEL_NAME)." >&2
  exit 1
fi
MODEL="$BASELINE_RESOLVED_MODEL_NAME"

if [[ -n "$PROMPT" && "$PROMPT" != "$BASELINE_PROMPT" ]]; then
  echo "Il placement sweep è ancorato alla baseline $BASELINE_ID. Il prompt richiesto non coincide con il prompt fisso di baseline." >&2
  exit 1
fi
