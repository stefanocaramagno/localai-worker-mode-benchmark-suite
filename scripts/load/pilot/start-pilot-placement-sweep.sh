...
print_usage() {
  cat <<'USAGE'
Usage:
  ./start-pilot-placement-sweep.sh --scenario PL1|PL2 --replica A|B|C [options]
...
validate_placement_scenario() {
  case "$1" in
    PL1|PL2) ;;
    *)
      echo "Unsupported placement scenario: $1" >&2
      exit 1
      ;;
  esac
}
...
if missing:
    print(
        f"The scenario configuration file '{scenario_path}' does not contain the required properties: {', '.join(missing)}.",
        file=sys.stderr,
    )
    sys.exit(1)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo "Unrecognized argument: $1" >&2; print_usage >&2; exit 1 ;;
  esac
done

[[ -n "$SCENARIO" ]] || { echo "The Scenario parameter is required." >&2; print_usage >&2; exit 1; }
[[ -n "$REPLICA" ]] || { echo "The Replica parameter is required." >&2; print_usage >&2; exit 1; }
validate_placement_scenario "$SCENARIO"
...
[[ -n "$OUTPUT_ROOT" ]] || OUTPUT_ROOT="$REPO_ROOT/results/experimental-cycles/C0/benchmark/pilot/placement"
...
[[ -n "$PRECHECK_CONFIG" ]] || PRECHECK_CONFIG="$REPO_ROOT/config/precheck/profiles/TC_C0_HISTORICAL_FIXED_CLUSTER.json"
[[ -n "$PHASE_CONFIG" ]] || PHASE_CONFIG="$REPO_ROOT/config/phases/profiles/WM_STANDARD_WARMUP_MEASUREMENT.json"
[[ -n "$PROTOCOL_CONFIG" ]] || PROTOCOL_CONFIG="$REPO_ROOT/config/protocol/profiles/EP_STANDARD_BENCHMARK_PROTOCOL.json"
[[ -n "$CLUSTER_CAPTURE_CONFIG" ]] || CLUSTER_CAPTURE_CONFIG="$REPO_ROOT/config/cluster-capture/profiles/CS_STANDARD_CLUSTER_CAPTURE.json"
[[ -n "$METRIC_SET_CONFIG" ]] || METRIC_SET_CONFIG="$REPO_ROOT/config/metric-set/profiles/MS_STANDARD_BENCHMARK_METRICS.json"
...
[[ -f "$BASELINE_CONFIG" ]] || { echo "The baseline file does not exist: $BASELINE_CONFIG" >&2; exit 1; }
if [[ "$SKIP_PRECHECK" != true && ! -f "$PRECHECK_SCRIPT" ]]; then echo "The pre-check script does not exist: $PRECHECK_SCRIPT" >&2; exit 1; fi
[[ -f "$PHASE_HELPER" ]] || { echo "The phase helper script does not exist: $PHASE_HELPER" >&2; exit 1; }
[[ -f "$PROTOCOL_HELPER" ]] || { echo "The protocol helper script does not exist: $PROTOCOL_HELPER" >&2; exit 1; }
[[ -f "$CLUSTER_CAPTURE_HELPER" ]] || { echo "The cluster-side collection helper script does not exist: $CLUSTER_CAPTURE_HELPER" >&2; exit 1; }
[[ -f "$METRIC_SET_HELPER" ]] || { echo "The metric-set helper script does not exist: $METRIC_SET_HELPER" >&2; exit 1; }
[[ -f "$PHASE_CONFIG" ]] || { echo "The warm-up/measurement profile file does not exist: $PHASE_CONFIG" >&2; exit 1; }
[[ -f "$PROTOCOL_CONFIG" ]] || { echo "The protocol file does not exist: $PROTOCOL_CONFIG" >&2; exit 1; }
[[ -f "$CLUSTER_CAPTURE_CONFIG" ]] || { echo "The cluster-side collection file does not exist: $CLUSTER_CAPTURE_CONFIG" >&2; exit 1; }
[[ -f "$METRIC_SET_CONFIG" ]] || { echo "The metric-set file does not exist: $METRIC_SET_CONFIG" >&2; exit 1; }
[[ -f "$CLUSTER_CAPTURE_SCRIPT" ]] || { echo "The cluster-side collection script does not exist: $CLUSTER_CAPTURE_SCRIPT" >&2; exit 1; }
...
if [[ "$API_SMOKE_ENABLED" == "true" && ! -f "$API_SMOKE_SCRIPT" ]]; then echo "The API smoke script does not exist: $API_SMOKE_SCRIPT" >&2; exit 1; fi

PLACEMENT_SCENARIO_FILE="$PLACEMENT_SCENARIO_CONFIG_ROOT/${SCENARIO}.json"
[[ -f "$PLACEMENT_SCENARIO_FILE" ]] || { echo "The scenario configuration file does not exist: $PLACEMENT_SCENARIO_FILE" >&2; exit 1; }
get_json_properties "$PLACEMENT_SCENARIO_FILE" "scenarioId,purpose,placementType,outputSubdir,referenceBaselineId,topologyDir"
PLACEMENT_SCENARIO_FILE_RESOLVED="${JSON_VALUES[0]}"
PLACEMENT_SCENARIO_ID="${JSON_VALUES[1]}"
PURPOSE="${JSON_VALUES[2]}"
PLACEMENT_TYPE="${JSON_VALUES[3]}"
OUTPUT_SUBDIR="${JSON_VALUES[4]}"
REFERENCE_BASELINE_ID="${JSON_VALUES[5]}"
TOPOLOGY_DIR_REL="${JSON_VALUES[6]}"

get_json_properties "$BASELINE_CONFIG" "baselineId,purpose,workerMode,modelScenario,resolvedModelName,workerScenario,resolvedWorkerCount,workloadScenario,namespaceManifest,storageManifest,prompt,temperature,requestTimeoutSeconds"
...
BASELINE_MODEL_SCENARIO="${JSON_VALUES[4]}"
BASELINE_RESOLVED_MODEL_NAME="${JSON_VALUES[5]}"
BASELINE_WORKER_SCENARIO="${JSON_VALUES[6]}"
BASELINE_RESOLVED_WORKER_COUNT="${JSON_VALUES[7]}"
BASELINE_WORKLOAD_SCENARIO="${JSON_VALUES[8]}"
BASELINE_NAMESPACE_MANIFEST_REL="${JSON_VALUES[9]}"
BASELINE_STORAGE_MANIFEST_REL="${JSON_VALUES[10]}"
BASELINE_PROMPT="${JSON_VALUES[11]}"
BASELINE_TEMPERATURE="${JSON_VALUES[12]}"
BASELINE_REQUEST_TIMEOUT_SECONDS="${JSON_VALUES[13]}"

[[ "$REFERENCE_BASELINE_ID" == "$BASELINE_ID" ]] || { echo "The placement scenario $SCENARIO requires baseline $REFERENCE_BASELINE_ID but the provided file exposes $BASELINE_ID." >&2; exit 1; }

MODEL_SCENARIO_FILE="$MODEL_SCENARIO_CONFIG_ROOT/${BASELINE_MODEL_SCENARIO}.json"
...
[[ -f "$MODEL_SCENARIO_FILE" ]] || { echo "The model scenario configuration file does not exist: $MODEL_SCENARIO_FILE" >&2; exit 1; }
...
MODEL_SCENARIO_ID="${JSON_VALUES[1]}"
MODEL_PURPOSE="${JSON_VALUES[2]}"
MODEL_NAME_FROM_FILE="${JSON_VALUES[3]}"
SERVER_MANIFEST_REL="${JSON_VALUES[6]}"

[[ -f "$WORKER_SCENARIO_FILE" ]] || { echo "The worker scenario configuration file does not exist: $WORKER_SCENARIO_FILE" >&2; exit 1; }
...
WORKER_SCENARIO_ID="${JSON_VALUES[1]}"
WORKER_PURPOSE="${JSON_VALUES[2]}"
WORKER_COUNT="${JSON_VALUES[3]}"

[[ -f "$WORKLOAD_SCENARIO_FILE" ]] || { echo "The workload scenario configuration file does not exist: $WORKLOAD_SCENARIO_FILE" >&2; exit 1; }
...
WORKLOAD_SCENARIO_ID="${JSON_VALUES[1]}"
WORKLOAD_PURPOSE="${JSON_VALUES[2]}"
USERS="${JSON_VALUES[3]}"
SPAWN_RATE="${JSON_VALUES[4]}"
RUN_TIME="${JSON_VALUES[5]}"

[[ "$MODEL_SCENARIO_ID" == "$BASELINE_MODEL_SCENARIO" && "$MODEL_NAME_FROM_FILE" == "$BASELINE_RESOLVED_MODEL_NAME" ]] || { echo "The placement baseline is not consistent with the reference model scenario." >&2; exit 1; }
[[ "$WORKER_SCENARIO_ID" == "$BASELINE_WORKER_SCENARIO" && "$WORKER_COUNT" == "$BASELINE_RESOLVED_WORKER_COUNT" ]] || { echo "The placement baseline is not consistent with the reference worker scenario." >&2; exit 1; }
[[ "$WORKLOAD_SCENARIO_ID" == "$BASELINE_WORKLOAD_SCENARIO" ]] || { echo "The placement baseline is not consistent with the reference workload scenario." >&2; exit 1; }

if [[ -n "$MODEL" && "$MODEL" != "$BASELINE_RESOLVED_MODEL_NAME" ]]; then
  echo "The placement sweep is anchored to baseline $BASELINE_ID. The requested model ($MODEL) does not match the fixed baseline model ($BASELINE_RESOLVED_MODEL_NAME)." >&2
  exit 1
fi
MODEL="$BASELINE_RESOLVED_MODEL_NAME"

if [[ -n "$PROMPT" && "$PROMPT" != "$BASELINE_PROMPT" ]]; then
  echo "The placement sweep is anchored to baseline $BASELINE_ID. The requested prompt does not match the fixed baseline prompt." >&2
  exit 1
fi
...
metric_set_write_files "$METRIC_SET_MANIFEST_PATH" "$METRIC_SET_TEXT_PATH" "Start-PilotPlacementSweep" "$RUN_ID" "$PHASE_MEASUREMENT_CSV_PREFIX" "$METRIC_SET_CLIENT_SOURCE_ARTIFACTS_JSON" "$METRIC_SET_CLUSTER_SOURCE_ARTIFACTS_JSON"
...
protocol_write_files "$PROTOCOL_MANIFEST_PATH" "$PROTOCOL_TEXT_PATH" "Start-PilotPlacementSweep" "$RUN_ID" \
...
echo "============================================="
echo " Official Pilot Placement Sweep Launcher"
echo "============================================="
echo "Repository                   : $REPO_ROOT"
echo "Placement scenario           : $SCENARIO"
...
echo "Resolved placement           : $PLACEMENT_TYPE"
...
if [[ "$AUTO_APPLY_K8S" == true ]]; then
  echo "Automatically applying the recommended Kubernetes targets before the run."
  for target in "${K8S_APPLY_TARGETS[@]}"; do
...
if [[ $EXIT_CODE -ne 0 ]]; then
  echo "The Locust run finished with exit code $EXIT_CODE." >&2
  exit $EXIT_CODE
fi

echo "The final cluster-side collection finished with exit code $CLUSTER_CAPTURE_POST_EXIT_CODE." >&2
