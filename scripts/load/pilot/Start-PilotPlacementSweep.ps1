...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) { $OutputRoot = Join-Path $repoRoot "results\experimental-cycles\C0\benchmark\pilot\placement" }
...
if ($referenceBaselineId -ne [string]$baselineConfigData.baselineId) {
    throw "The placement scenario $Scenario requires baseline $referenceBaselineId but the provided file exposes $($baselineConfigData.baselineId)."
}
...
if ([string]$modelScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.modelScenario -or [string]$modelScenarioData.RawConfig.modelName -ne [string]$baselineConfigData.resolvedModelName) {
    throw "The placement baseline is not consistent with the reference model scenario."
}
...
if ([string]$workerScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.workerScenario -or [int]$workerScenarioData.RawConfig.workerCount -ne [int]$baselineConfigData.resolvedWorkerCount) {
    throw "The placement baseline is not consistent with the reference worker scenario."
}

if ([string]$workloadScenarioData.RawConfig.scenarioId -ne [string]$baselineConfigData.workloadScenario) {
    throw "The placement baseline is not consistent with the reference workload scenario."
}

if (-not [string]::IsNullOrWhiteSpace($Model) -and $Model -ne [string]$baselineConfigData.resolvedModelName) {
    throw "The placement sweep is anchored to baseline $($baselineConfigData.baselineId). The requested model ($Model) does not match the fixed baseline model ($($baselineConfigData.resolvedModelName))."
}
...
if (-not [string]::IsNullOrWhiteSpace($Prompt) -and $Prompt -ne [string]$baselineConfigData.prompt) {
    throw "The placement sweep is anchored to baseline $($baselineConfigData.baselineId). The requested prompt does not match the fixed baseline prompt."
}
