...
get_scenario_config() {
  ...
  if [[ ! -f "$scenario_file" ]]; then
    echo "The scenario configuration file does not exist: $scenario_file" >&2
    exit 1
  fi
...
missing = [key for key in required if key not in data]
if missing:
    print(f"The scenario configuration file '{scenario_path}' does not contain the required properties: {', '.join(missing)}.", file=sys.stderr)
    sys.exit(1)
print(str(scenario_path))
for key in required:
    print(str(data[key]))
...
if [[ -z "$OUTPUT_ROOT" ]]; then
  OUTPUT_ROOT="$REPO_ROOT/results/experimental-cycles/C0/benchmark/pilot/workload"
fi
...
if [[ "$REFERENCE_BASELINE_ID" != "$BASELINE_ID" ]]; then
  echo "The workload scenario $SCENARIO requires baseline $REFERENCE_BASELINE_ID but the provided file exposes $BASELINE_ID." >&2
  exit 1
fi
...
if [[ "${JSON_VALUES[1]}" != "$BASELINE_MODEL_SCENARIO" || "${JSON_VALUES[2]}" != "$BASELINE_MODEL_NAME" || "${JSON_VALUES[3]}" != "$BASELINE_SERVER_MANIFEST_REL" ]]; then
  echo "The workload baseline is not consistent with the reference model scenario." >&2
  exit 1
fi
...
if [[ "${JSON_VALUES[1]}" != "$BASELINE_WORKER_SCENARIO" || "${JSON_VALUES[2]}" != "$BASELINE_WORKER_COUNT" ]]; then
  echo "The workload baseline is not consistent with the reference worker scenario." >&2
  exit 1
fi
...
if [[ "${JSON_VALUES[1]}" != "$BASELINE_PLACEMENT_SCENARIO" || "${JSON_VALUES[2]}" != "$BASELINE_PLACEMENT_TYPE" || "${JSON_VALUES[3]}" != "$BASELINE_TOPOLOGY_DIR_REL" ]]; then
  echo "The workload baseline is not consistent with the reference placement scenario." >&2
  exit 1
fi

if [[ -n "$MODEL" && "$MODEL" != "$BASELINE_MODEL_NAME" ]]; then
  echo "The workload sweep is anchored to baseline $BASELINE_ID. The requested model ($MODEL) does not match the fixed baseline model ($BASELINE_MODEL_NAME)." >&2
  exit 1
fi
MODEL="$BASELINE_MODEL_NAME"

if [[ -n "$PROMPT" && "$PROMPT" != "$BASELINE_PROMPT" ]]; then
  echo "The workload sweep is anchored to baseline $BASELINE_ID. The requested prompt does not match the fixed baseline prompt." >&2
  exit 1
fi
...
if [[ $EXIT_CODE -eq 0 && $CLUSTER_CAPTURE_POST_EXIT_CODE -eq 0 ]]; then
  ...
  if [[ $EXIT_CODE -ne 0 ]]; then
    echo "The Locust run finished with exit code $EXIT_CODE."
    exit $EXIT_CODE
  fi
  echo "The final cluster-side collection finished with exit code $CLUSTER_CAPTURE_POST_EXIT_CODE."
