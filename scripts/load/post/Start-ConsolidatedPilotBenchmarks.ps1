...
function Wait-ConsolidatedClusterStabilization {
    ...
    do {
        ...
        if ($deployments.Count -eq 0) {
            $lastReason = "No LocalAI deployment observed in namespace '$Namespace'."
            Start-Sleep -Seconds $PollIntervalSeconds
            continue
        }

        if ($pods.Count -eq 0) {
            $lastReason = "No LocalAI pod observed in namespace '$Namespace'."
            Start-Sleep -Seconds $PollIntervalSeconds
            continue
        }
        ...
        foreach ($pod in $pods) {
            ...
            if ($null -ne $deletionTimestamp) {
                $podIssues.Add(("{0}: terminating" -f $name)) | Out-Null
                continue
            }
            ...
            if ($null -eq $readyCondition -or [string](Get-OptionalObjectPropertyValue -InputObject $readyCondition -PropertyName "status" -DefaultValue "") -ne "True") {
                $podIssues.Add(("{0}: Ready condition not satisfied" -f $name)) | Out-Null
            }
        }
        ...
        if ($deploymentIssues.Count -eq 0 -and $podIssues.Count -eq 0) {
            return [pscustomobject]@{
                ...
                podCount = $pods.Count
                summary = "LocalAI deployments and pods are stable in namespace '$Namespace'."
            }
        }

        $issueParts = @()
        if ($deploymentIssues.Count -gt 0) {
            $issueParts += ("unstable deployments: {0}" -f (($deploymentIssues.ToArray()) -join "; "))
        }
        if ($podIssues.Count -gt 0) {
            $issueParts += ("unstable pods: {0}" -f (($podIssues.ToArray()) -join "; "))
        }
    ...
    throw "Timeout while waiting for cluster stabilization in namespace '$Namespace'. last observation: $lastReason"
}


if ([string]::IsNullOrWhiteSpace($ProfileConfig)) {
    $ProfileConfig = Join-Path $repoRoot "config\pilot-consolidation\profiles\PC_CONSOLIDATED_PILOT_CAMPAIGN.json"
}
...
foreach ($familyName in $targetFamilies) {
    ...
    if (-not (Test-Path $launcherPath)) {
        throw "Family launcher not found: $launcherPath"
    }
...
Write-Host "Campaign manifest    : $manifestPath"
Write-Host "Campaign report      : $textReportPath"
Write-Host "Rigor summary         : $rigorManifestPath"
Write-Host "Rigor report          : $rigorTextPath"
Write-Host "Campaign status        : $overallStatus"
