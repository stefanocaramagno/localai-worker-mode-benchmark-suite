...
wait_consolidated_cluster_stabilization() {
  ...
  while (( $(date +%s) < deadline )); do
    ...
    if ! get_kubectl_json_for_consolidation "$kubeconfig_path" get deployments -n "$namespace" > "$deployments_json_file"; then
      ...
      echo "Error while retrieving deployments for cluster stabilization." >&2
      return 1
    fi

    if ! get_kubectl_json_for_consolidation "$kubeconfig_path" get pods -n "$namespace" > "$pods_json_file"; then
      rm -f -- "$deployments_json_file" "$pods_json_file"
      echo "Error while retrieving pods for cluster stabilization." >&2
      return 1
    fi
...
if not deployments:
    print('WAIT	0	0	No LocalAI deployment observed in the specified namespace.')
    raise SystemExit(0)

if not pods:
    print('WAIT	{}	0	No LocalAI pod observed in the specified namespace.'.format(len(deployments)))
    raise SystemExit(0)
...
for pod in pods:
    ...
    if metadata.get('deletionTimestamp') is not None:
        pod_issues.append(f'{name}: terminating')
        continue
    ...
    if ready_condition is None or str(ready_condition.get('status', '')) != 'True':
        pod_issues.append(f'{name}: Ready condition not satisfied')

if not deployment_issues and not pod_issues:
    print(f'OK	{len(deployments)}	{len(pods)}	LocalAI deployments and pods are stable in the selected namespace.')
    raise SystemExit(0)

issue_parts = []
if deployment_issues:
    issue_parts.append('unstable deployments: ' + '; '.join(deployment_issues))
if pod_issues:
    issue_parts.append('unstable pods: ' + '; '.join(pod_issues))
...
PY2
  ...
  echo "Timeout while waiting for cluster stabilization in namespace '$namespace'. last observation: $last_reason" >&2
  return 1
}
...
if [[ -z "$PROFILE_CONFIG" ]]; then
  PROFILE_CONFIG="$REPO_ROOT/config/pilot-consolidation/profiles/PC_CONSOLIDATED_PILOT_CAMPAIGN.json"
fi
...
if [[ ! -f "$STATISTICAL_RIGOR_CONFIG" ]]; then
  echo "The statistical-rigor file does not exist: $STATISTICAL_RIGOR_CONFIG" >&2
  exit 1
fi
...
for family_name in "${TARGET_FAMILIES[@]}"; do
  ...
  if [[ ! -f "$family_launcher" ]]; then
    echo "Family launcher not found: $family_launcher" >&2
    exit 1
  fi
...
echo
echo "Campaign manifest    : $CAMPAIGN_MANIFEST_PATH"
echo "Campaign report      : $CAMPAIGN_TEXT_PATH"
echo "Rigor summary         : $RIGOR_MANIFEST_PATH"
echo "Rigor report          : $RIGOR_TEXT_PATH"
echo "Campaign status        : $OVERALL_STATUS"
