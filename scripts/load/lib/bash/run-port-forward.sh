...
wait_kubernetes_service_backend_ready() {
  ...
  echo "Timeout while waiting for Kubernetes readiness of service/$service_name in namespace $namespace." >&2
  return 1
}
...
ensure_local_kubernetes_port_forward() {
  ...
  if command -v python >/dev/null 2>&1 && python --version >/dev/null 2>&1; then
    ...
    echo "No compatible Python interpreter is available in PATH. Unable to automatically parse the Base URL for port-forwarding." >&2
    return 1
  fi
...
PY
  ...
  if port_forward_test_local_port_reachable 127.0.0.1 "$port" 1; then
    ...
    if (( ${#stopped[@]} > 0 )); then
      echo "Detected an existing local listener on localhost:${port}. Stopped residual kubectl processes before creating a new port-forward: $(IFS=', '; echo "${stopped[*]}")."
    fi
    if port_forward_test_local_port_reachable 127.0.0.1 "$port" 1; then
      echo "Local port localhost:${port} is already in use. A new port-forward cannot be created safely." >&2
      return 1
    fi
  fi

  command -v kubectl >/dev/null 2>&1 || { echo 'kubectl is not available in PATH. Unable to create the port-forward automatically.' >&2; return 1; }
  [[ -n "$namespace" ]] || { echo 'Namespace is required to automatically create the port-forward to the Kubernetes service.' >&2; return 1; }

  echo "Checking Kubernetes backend readiness for service/${service_name} in namespace ${namespace} before local port-forwarding."
  ...
  local attempt=1
  while (( attempt <= 3 )); do
    echo "Local port-forward is not reachable on ${host_name}:${port}. Automatic start: kubectl ${kubectl_args[*]} (attempt ${attempt})"
    ...
    while (( waited < 30 )); do
      ...
      if port_forward_test_local_port_reachable 127.0.0.1 "$port" 1; then
        echo "Port-forward active on ${host_name}:${port} (PID ${pf_pid}). Checking HTTP readiness on ${readiness_url}."
        local ready_wait=0
        ...
        echo "The backend was not ready to accept connections behind the port-forward yet; starting a new attempt..."
        break
      fi
      sleep 1
      waited=$((waited + 1))
  ...
  echo "Timeout while waiting for the local port-forward to service/$service_name on localhost:$port." >&2
  return 1
}
