...
if missing:
    print(f"The convention file '{path}' does not contain the required properties: {', '.join(missing)}.", file=sys.stderr)
    sys.exit(1)
...
rc_validate_component() {
  ...
  if [[ ! "$component_value" =~ ^[A-Za-z0-9._-]+$ ]]; then
    echo "Error: component '$component_name' contains invalid characters: $component_value" >&2
    exit 1
  fi
}
