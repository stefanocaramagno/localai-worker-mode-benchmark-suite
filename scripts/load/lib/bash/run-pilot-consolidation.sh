...
if missing:
    print(
        f"The consolidation file '{profile_path}' does not contain the required properties: {', '.join(missing)}.",
        file=sys.stderr,
    )
    sys.exit(1)
...
if family_name not in families:
    print(f"The family '{family_name}' is not defined in profile '{profile_path}'.", file=sys.stderr)
    sys.exit(1)
...
if missing:
    print(
        f"The family '{family_name}' in profile '{profile_path}' does not contain the required properties: {', '.join(missing)}.",
        file=sys.stderr,
    )
    sys.exit(1)
