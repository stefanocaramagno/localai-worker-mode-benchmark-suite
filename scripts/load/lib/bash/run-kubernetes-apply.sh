...
invoke_k8s_apply_target() {
  ...
  case "$kind" in
    file)
      echo "Automatic Kubernetes target apply: kubectl${kubeconfig_path:+ --kubeconfig $kubeconfig_path} apply -f $path"
      kubectl "${kubectl_args[@]}" apply -f "$path"
      ;;
    directory)
      echo "Automatic Kubernetes target apply: kubectl${kubeconfig_path:+ --kubeconfig $kubeconfig_path} apply -k $path"
      kubectl "${kubectl_args[@]}" apply -k "$path"
      ;;
    *) echo "Invalid or unresolved Kubernetes target: $path" >&2; exit 1 ;;
  esac
}
