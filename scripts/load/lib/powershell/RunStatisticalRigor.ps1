...
function Get-StatisticalRigorProfile {
    ...
    if (-not (Test-Path $ProfileFile)) {
        throw "The statistical-rigor file does not exist: $ProfileFile"
    }
    ...
    foreach ($propertyName in $required) {
        if (-not ($data.PSObject.Properties.Name -contains $propertyName)) {
            throw "The statistical-rigor file '$ProfileFile' does not contain the required property '$propertyName'."
        }
    }
...
'@
    ...
    $artifactRoot = Get-ArtifactPortabilityRepositoryRoot -ReferencePath $OutputManifestPath
    $portableEntries = ConvertTo-PortableArtifactObject -InputObject @($Entries) -RepositoryRoot $artifactRoot
    $portableProfilePayload = ConvertTo-PortableArtifactObject -InputObject $profilePayload -RepositoryRoot $artifactRoot

    $entriesJson = @($portableEntries) | ConvertTo-Json -Depth 12 -Compress
    $profileJson = $portableProfilePayload | ConvertTo-Json -Depth 12 -Compress
    ...
    try {
        ...
        if ($null -eq $pythonCommand) {
            throw "Python is not available in PATH. Unable to generate the statistical-rigor summary."
        }
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Statistical-rigor summary generation finished with exit code $LASTEXITCODE."
        }
    }
    finally {
