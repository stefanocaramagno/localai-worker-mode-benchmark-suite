...
if [[ -z "$OUTPUT_PREFIX" ]]; then
  OUTPUT_PREFIX="$REPO_ROOT/results/experimental-cycles/C0/validation/minimal-metrics"
fi
...
from artifact_paths import normalize_artifact_payload
...
with open(json_path, "w", encoding="utf-8-sig") as fh:
    json.dump(normalize_artifact_payload(summary, repo_root), fh, indent=2, ensure_ascii=False)
    fh.write("\n")
