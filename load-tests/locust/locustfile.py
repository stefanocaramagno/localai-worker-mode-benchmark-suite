...
def _read_non_negative_float_env(variable_name: str, default_value: str) -> float:
    ...
    try:
        ...
        raise ValueError(
            f"Environment variable {variable_name} must be a numeric value, got {raw_value!r}"
        ) from exc

    if parsed_value < 0:
        raise ValueError(
            f"Environment variable {variable_name} must be greater than or equal to 0, got {parsed_value}"
        )
...
WAIT_TIME_SECONDS = _read_non_negative_float_env("LOCALAI_WAIT_TIME_SECONDS", "2")
LOCALAI_HTTP_HOST_HEADER = os.getenv("LOCALAI_HTTP_HOST_HEADER", "").strip()
LOCALAI_HTTP_HEADERS = {"Host": LOCALAI_HTTP_HOST_HEADER} if LOCALAI_HTTP_HOST_HEADER else {}
