...
- rerun preparation for baseline, exploratory, pilot, consolidated, diagnosis, and completion-gate phases.
...
3. **Reset workload state, not project state.** Benchmark artifacts under `results/` must remain available unless a deliberate archival or replacement action has been taken.
