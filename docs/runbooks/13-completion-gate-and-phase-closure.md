...
This runbook defines how to evaluate whether the current exploratory characterization cycle can be formally considered complete after technical diagnosis and advisor-facing reporting have been generated.
...
The completion gate does not execute workload generation, Kubernetes deployment, smoke validation, diagnosis logic, or report generation by itself. Instead, it evaluates whether those upstream phases produced enough structured evidence to support a disciplined phase closure.
...
- the reporting and visualization phase has already generated the required advisor-facing artifacts;
...
- advisor-facing reporting artifacts exist under `results/reporting/`.
...
- required reporting artifacts and sweep reports.
...
- reporting artifacts generated under `results/reporting/`;
- sweep-level reporting artifacts generated under `results/reporting/sweeps/`;
...
- reporting artifacts required: yes
- required reporting root: `results/reporting`
- required global report files: `index.html`, `report.md`, `scenario-summary.csv`, `reporting-manifest.json`
- required sweep reports: `worker-count`, `workload`, `models`, `placement`
- required chart metrics: mean response time, P50, P95, P99, and throughput/RPS
- reporting archive: optional and not required for completion-gate success
...
- the reporting and visualization phase has already completed.
...
- reporting outputs are present under `results/reporting/` and are internally readable.
...
The completion gate consumes the diagnosis JSON as its primary analytical input and verifies required reporting artifacts as closure evidence.
...
In practice, the completion gate is **diagnosis-driven and reporting-aware**. This means the technical diagnosis phase must already have transformed the raw benchmark campaign into structured analytical metadata, and the reporting phase must already have transformed that evidence into advisor-facing reports.

---

## Gate Logic Evaluated by `CG1`

The completion gate evaluates the configured gates, including the availability of technical diagnosis, coverage evidence, cluster-side evidence, and required reporting outputs.
...
## Reporting Artifact Checks

The current completion-gate profile requires the reporting phase to have produced a stable advisor-facing output directory. The gate verifies the configured reporting root and expected artifact names instead of requiring the operator to pass those paths manually.

The required reporting outputs include:
...
These checks ensure that the benchmark campaign is not only technically diagnosed, but also reviewable through readable reports and charts.
...
Run the completion gate from the repository root. The standard invocation has no mandatory arguments:
...
- `config/completion-gate/CG1.json` as the active profile;
- the latest `all` diagnosis JSON under `results/diagnosis/`.

Explicit overrides remain available for retrospective evaluation:
...
The standard Bash invocation also has no mandatory arguments:

```bash
./scripts/load/post/start-completion-gate.sh
```

Explicit overrides remain available when a specific profile or diagnosis must be evaluated:
...
```bash
./scripts/load/post/start-completion-gate.sh --dry-run
```
...
2. Confirm that the reporting phase has already generated `results/reporting/`.
3. Confirm that the diagnosis corresponds to the current benchmark campaign.
4. Confirm that all required consolidated pilot families are present.
5. Run the completion gate.
6. Inspect the text summary first.
7. Inspect the JSON manifest when one or more gates fail.
8. Decide whether to:
...
- required reporting artifacts are present.
